...
ALTER TABLE rsvp ADD COLUMN group_id INTEGER REFERENCES rsvp_group(id);
...
ALTER TABLE rsvp DROP COLUMN food_allergies;

ALTER TABLE rsvp DROP COLUMN email;

-- Remove the accompanying_guests as this is not needed.
ALTER TABLE rsvp DROP COLUMN accompanying_guests;

-- Remove the need for passcode as fetching rsvp by passcode is no longer desired. Use serial id for uniqueness.
ALTER TABLE rsvp DROP COLUMN passcode;

ALTER TABLE rsvp ADD COLUMN id SERIAL PRIMARY KEY;
